...
const BuilderAttendanceChart = ({ timeRange, data }) => {
  ...
  const processChartData = (rawData) => {
    try {
      ...
      console.log('Processing builder attendance data:', rawData);
      ...
      const labels = rawData.rows.map(row => {
        const date = new Date(row.f[0].v); // week_start field
      ...
      console.error('Error processing builder attendance data:', err);
      setError(`Failed to process builder attendance data: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Weekly Active Users and Completion Rate',
        font: {
          size: 16,
          weight: 'bold'
        }
...
export default BuilderAttendanceChart; 