...
app.get('/api/builders', async (req, res) => {
  ...
  // Querying the pre-aggregated table generated by the scheduled query
  const aggregatedTable = `${PROJECT_ID}.${DATASET}.daily_builder_metrics`; // *** Use the correct table name ***
  ...
  const query = `
    WITH AggregatedResults AS (
        SELECT
            ...
            -- Calculate overall averages from daily sums/counts
            ...
            SAFE_DIVIDE(SUM(sum_work_product_score_today), SUM(count_work_product_score_today)) as avg_work_product_score, 
            SAFE_DIVIDE(SUM(sum_comprehension_score_today), SUM(count_comprehension_score_today)) as avg_comprehension_score
    ...
    SELECT 
      user_id,
      name,
      ROUND((total_completed_tasks / NULLIF(total_attempted_tasks, 0)) * 100, 2) as tasks_completed_percentage,
      total_prompts_sent as prompts_sent,
      CASE 
        WHEN avg_daily_sentiment IS NULL THEN NULL
      ...
      CASE 
        WHEN avg_peer_feedback_sentiment IS NULL THEN NULL
      ...
      ROUND(avg_work_product_score, 2) as work_product_score,
      ROUND(avg_comprehension_score, 2) as comprehension_score
...
app.get('/api/builders/:userId/details', async (req, res) => {
  ...
  if (type === 'workProduct') {
    ...
    // Optimized to count prompts per day directly in BQ
    query = `
      SELECT
